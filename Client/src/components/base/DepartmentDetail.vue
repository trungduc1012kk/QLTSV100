    ...
    <div class="form-detail-container">
      ...
      <div class="form-detail-body">
        <div class="form-detail-content">
          <div class="row">
            <label for="">Mã phòng ban <span>*</span></label>
            <div class="form-input">
              <input
                ...
                :class="error.departmentCode != '' ? 'border-red' : ''"
                @blur="validate('departmentCode')"
                ref="propertyCode"
          ...
          <div class="row">
            <label for="">Tên phòng ban <span>*</span></label>
            <div class="form-input">
              <input
                class="input"
                type="text"
                ...
                :class="error.departmentName != '' ? 'border-red' : ''"
                @blur="validate('departmentName')"
                ref="propertyName"
              />
            </div>
            <p class="error" v-if="error.departmentName != ''">
              <span>Tên phòng ban </span>{{ error.departmentName }}
            </p>
          </div>
          <div class="row col-2">
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        departmentName: { Required: true, MaxLength: 255 }
  ...
  methods: {
     ...
     */
     addProperty() {
      axios
        .post(`${BASE_URL_Department}`, this.property)
        .then((response) => {
        ...
        .catch((err) => {
          ...
          if (err.response.data.errorCode == ErrorCode.DuplicateCode) {
            setTimeout(() => {
              this.error.departmentCode = ErrorMsg.Duplicate;
              this.focusToError();
            }, 500);
          }
        });
    ...
    getNewPropertyCode() {
      axios({
        url: `${BASE_URL_Department}/new-code`,
        method: "Get",
      })
        .then((response) => {
          this.property.departmentCode = response.data ? response.data.toString() : "";
        })
        .catch((err) => {
    ...
    editProperty() {
      axios
        .put(`${BASE_URL_Department}/${this.property.departmentID}`, this.property)
        .then((response) => {
          console.log(response);
          this.$emit("success");
        })
  ...
  created() {
    ...
    if (this.commandName != CommandName.Edit) {
      ...
      this.property.departmentID = GuidEmpty;
    } else {
